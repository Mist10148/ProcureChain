...
#### Simulated Five-Node Blockchain with Hash-Linking
...
When a critical action occurs — document upload, approval decision, rejection, or budget publication — the system reads the latest block to determine the next sequential index and the preceding hash, computes the new block's hash, and simultaneously appends the new block row to all five node files. Validation cross-checks all five nodes against a reference chain derived from node 1, reporting per-node chain integrity status, the first mismatch index per node, and per-block consensus counts representing the number of nodes that agree on each block's hash value.
